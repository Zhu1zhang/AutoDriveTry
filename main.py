...
def main():
    ...
    print(f"  中心线点数: {len(centerline)}, 半宽: {config.TRACK['half_width']}")
    ...
    if not args.skip_pso:
        ...
        best_params, fitness_history = run_pso(track, n_iters=config.PSO["n_iters"], n_particles=config.PSO["n_particles"])
